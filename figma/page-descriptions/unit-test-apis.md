# Unit Testing the API Functions
...
Now that the `initializeTimes` function calls the `fetchAPI` function, the unit tests need to be updated. If you run the tests now, you will discover that the existing test is failing. For testing purposes, mock `fetchAPI` to return a non-empty array of available booking times.

### Step 2: Update the test for `updateTimes`

Similar to the previous step, the test you previously created for `updateTimes` will fail. Update the test to include a pre-selected date as part of the dispatch data.
